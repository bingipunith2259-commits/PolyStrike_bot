...
TIERS = {
    "basic": {
        ...
        "daily":      10,
        "days":       30,
        "label":      "Basic — 10 analyses/day",
        "perks":      "10 analyses per day, market search, trending markets",
    },
    "pro": {
        "name":       "🥈 Pro",
        "stars":      1000,
        "daily":      50,
        "days":       30,
        "label":      "Pro — 50 analyses/day",
        "perks":      "50 analyses per day, priority AI, full breakdown",
    },
    "whale": {
        "name":       "🐳 Whale",
        "stars":      2500,
        "daily":      999999,
        "days":       30,
        "label":      "Whale — Unlimited analyses",
        "perks":      "Unlimited analyses, fastest AI, VIP badge",
    },
}

FREE_ANALYSES = 3   # one-time free trial
...
def apply_subscription(user_id: int, tier_key: str):
    tier = TIERS[tier_key]
    expires = (datetime.utcnow() + timedelta(days=tier["days"])).isoformat()
    con = sqlite3.connect(DB_PATH)
    con.execute("""
        UPDATE users SET tier=?, expires_at=?, daily_used=0, last_reset=?
        WHERE user_id=?
...
def check_and_reset_daily(user: dict) -> dict:
    ...
    if user["last_reset"] != today:
        con = sqlite3.connect(DB_PATH)
        con.execute("UPDATE users SET daily_used=0, last_reset=? WHERE user_id=?",
                    (today, user["user_id"]))
        con.commit()
        con.close()
        user["daily_used"] = 0
...
def can_analyze(user: dict) -> tuple[bool, str]:
...
def record_usage(user_id: int, is_free: bool = False):
    con = sqlite3.connect(DB_PATH)
    if is_free:
        con.execute("""
            UPDATE users SET free_used=free_used+1, total_used=total_used+1 WHERE user_id=?
        """, (user_id,))
    else:
        con.execute("""
            UPDATE users SET daily_used=daily_used+1, total_used=total_used+1 WHERE user_id=?
        """, (user_id,))
    con.commit()
    con.close()
...
def ai_analyze(market, outcomes, tier_key="free"):
    ...
    prompt = f"""You are PolyBot, an expert prediction market analyst. Analyze this Polymarket market.
...
Respond in this exact format (punchy, max 300 words):

VERDICT
[1-2 sentences: most likely outcome and why]

BREAKDOWN
[One line per outcome explaining the pricing]
...
CONFIDENCE SIGNAL
[HIGH / MEDIUM / LOW — one sentence based on volume and liquidity]

RISK NOTE
[Biggest tail risk — one sentence]"""
...
def tier_badge(tier_key):
    return TIERS.get(tier_key, {}).get("name", "🆓 Free")
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "👋 *Welcome to PolyBot!*\n\n"
        "I analyze Polymarket prediction markets using live AI — "
        "so you know the most probable outcome before you bet.\n\n"
...
async def cmd_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if active:
        ...
        text = (
            f"*Your Plan: {tier_badge(u['tier'])}*\n\n"
            f"📅 Expires in: *{days_left} days*\n"
            f"⚡ Analyses left today: *{daily_left}*\n"
            f"📊 Total analyses done: *{u['total_used']}*\n\n"
            "Use /subscribe to renew or upgrade."
        )
    else:
        free_left = max(0, FREE_ANALYSES - u["free_used"])
        text = (
            f"*Your Plan: 🆓 Free Trial*\n\n"
            f"🎁 Free analyses left: *{free_left}/{FREE_ANALYSES}*\n"
            f"📊 Total analyses done: *{u['total_used']}*\n\n"
...
async def cmd_subscribe(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "  └ Unlimited analyses\n\n"
        "⭐ Stars are bought inside Telegram.\n"
...
async def handle_buy_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
...
async def handle_precheckout(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Telegram requires you to answer pre-checkout within 10s."""
    await update.pre_checkout_query.answer(ok=True)


async def handle_successful_payment(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    payment = update.message.successful_payment
    payload = payment.invoice_payload               # e.g. "sub_pro_12345"
    ...
    await update.message.reply_text(
        f"✅ *Payment confirmed!*\n\n"
        f"Welcome to {tier['name']} 🎉\n\n"
        f"• {tier['perks']}\n"
        f"• Valid for 30 days\n\n"
        f"Use /status to check your plan anytime.",
        parse_mode="Markdown"
    )
...
async def cmd_trending(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not markets:
        await update.message.reply_text("⚠️ Couldn't fetch trending markets. Try again later.")
        return
    keyboard = []
...
async def handle_analyze_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    if not allowed:
        if reason == "no_sub":
            ...
            await query.edit_message_text(
                f"🔒 *You've used all {FREE_ANALYSES} free analyses.*\n\n"
                "Subscribe to keep going — plans start at just 500 ⭐/month (~$7).\n\n"
            ...
            await query.edit_message_text(
                ...
                f"Your {tier_badge(u['tier'])} plan includes {tier.get('daily', 0)} analyses/day.\n"
    ...
    if not market:
        await query.edit_message_text("❌ Could not fetch market data. Try again.")
        return

    outcomes = parse_outcomes(market)
    if not outcomes:
        await query.edit_message_text("⚠️ No outcome data available for this market.")
        return
    ...
    analysis = ai_analyze(market, outcomes, u["tier"])

    # Footer
    if is_free:
        free_left = max(0, FREE_ANALYSES - u["free_used"])
        footer = f"\n\n_🎁 Free analyses left: {free_left}. Subscribe with /subscribe_"
    elif u["tier"] != "whale":
        tier     = TIERS[u["tier"]]
...
async def handle_show_plans_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    keyboard = [
        [InlineKeyboardButton("🥉 Basic — 500 ⭐ /month",  callback_data="buy:basic")],
